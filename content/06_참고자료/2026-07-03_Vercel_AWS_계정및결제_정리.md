...
### 6-3. 처음부터 팀 계정으로 시작하는 것을 권장

| 항목 | 개인 계정으로 시작 | 팀 Gmail로 시작 |
|------|-----------------|----------------|
| Vercel 인수인계 | Owner 교체로 가능 | 루트 이메일만 교체 |
| AWS 인수인계 | 루트 이메일 교체 필요 | 루트 이메일만 교체 |
| 리스크 | 개인 계정 분실·퇴사 시 접근 불가 위험 | 팀이 계정 공유 가능 |

**결론:** 팀 Gmail(`framewave.ax@gmail.com`)로 Vercel·AWS를 최초 생성하고, 인수인계 시 루트 이메일을 회사 이메일로 교체하는 방식이 가장 안전하고 간단하다.
