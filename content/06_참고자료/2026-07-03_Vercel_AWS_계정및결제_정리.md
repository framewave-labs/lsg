...
인턴십 종료(07.29) 후 회사에 원활하게 인수인계하기 위한 계정 이전 방법 정리.

### 6-1. Vercel — 인수인계 원활

팀(Team) 단위로 프로젝트·배포·설정이 귀속되어 있어 소유권 이전이 깔끔하다.

**이전 절차**
1. 회사 계정을 팀에 **Owner**로 초대
2. 기존 개인(또는 팀 Gmail) 계정을 팀에서 제거
3. 결제 수단을 회사 카드로 교체

→ 프로젝트·배포 이력·환경변수 설정이 팀에 귀속되어 있으므로 데이터 손실 없이 이전 완료.

### 6-2. AWS — 루트 계정 정보 교체 방식 권장

AWS는 Vercel처럼 팀 멤버 교체로 소유권을 이전하는 구조가 아니다. **계정 자체가 곧 소유 단위**이며, 루트 이메일 하나에 모든 리소스가 귀속된다.

| 방법 | 설명 | 난이도 |
|------|------|--------|
| **루트 이메일·결제 수단 변경** | Account Settings에서 루트 이메일과 결제 수단을 회사 것으로 교체 | 쉬움 (권장) |
| AWS Organizations로 편입 | 회사 org에 계정을 이전 | 복잡 (BE 직접 처리 필요) |

→ 인수인계 목적에는 **루트 이메일 변경이 가장 현실적**. EC2·S3 등 리소스와 설정이 그대로 유지된다.
- 근거: [Update the root user email address — AWS 공식 문서](https://docs.aws.amazon.com/accounts/latest/reference/manage-acct-update-root-email.html)

### 6-3. AWS 계정 운영 방향 (확정)

팀 공용 Gmail을 별도로 만들기 어려운 상황이므로, Vercel과 동일한 방식으로 운영한다.

**확정 방향: BE 담당자 개인 이메일로 AWS 계정 생성 → 인수인계 시 회사 이메일로 루트 교체**

1. BE 담당자(김유신 또는 송영범) 개인 이메일로 AWS 계정 생성 및 운영
2. 인수인계 시점에 AWS Account Settings에서 루트 이메일을 회사 이메일로 변경
3. 루트 MFA 재설정 (기존 담당자 MFA 해제 → 회사 기준으로 재등록)
4. 결제 수단을 회사 카드로 교체

**이 방향을 선택한 이유**
- 팀 공용 Gmail 신규 발급이 현실적으로 어려운 상황
- AWS는 루트 이메일 변경만으로 EC2·S3 등 모든 리소스와 설정이 그대로 유지됨 — 재생성·마이그레이션 불필요
- Vercel(FE 개인 계정 시작)과 인수인계 절차가 동일해져 관리 일관성 확보
- AWS Organizations 편입보다 절차가 단순하고, 인턴십 기간(4주) 내 운영하는 규모에 적합

- 근거: [Update the root user email address — AWS 공식 문서](https://docs.aws.amazon.com/accounts/latest/reference/manage-acct-update-root-email.html)

### 6-4. Vercel 계정 운영 방향 (확정)

Vercel은 팀 구조이므로 개인 계정으로 시작해도 소유권 이전이 깔끔하게 가능하다.

**확정 방향: FE 개인 계정으로 팀 생성 → 인수인계 시 회사 계정으로 이전**

1. FE(김은재) 개인 계정으로 Vercel 팀 생성 및 프로젝트 운영
2. 인수인계 시점에 회사 계정을 팀에 **Owner**로 초대
3. FE 개인 계정을 팀에서 제거
4. 결제 수단을 회사 카드로 교체

→ 프로젝트·배포 이력·환경변수가 팀에 귀속되어 있으므로 데이터 손실 없이 이전 완료.

### 6-5. 인수인계 시 전달 항목 (체크리스트)
...
**AWS**
- [ ] 루트 이메일·비밀번호 전달 (또는 교체)
- [ ] 루트 MFA 재설정
- [ ] 결제 수단 교체 완료
- [ ] IAM 계정 목록 정리 (팀원 계정 정리 또는 이전)
- [ ] 사용 중인 서비스 목록 전달 (EC2, S3, 기타)
- [ ] 환경변수·API 키 목록 전달
...
- [Update the root user email address](https://docs.aws.amazon.com/accounts/latest/reference/manage-acct-update-root-email.html)
